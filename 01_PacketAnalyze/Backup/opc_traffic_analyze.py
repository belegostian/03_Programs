...
SESSION_NUM = 4                                                         # (int): 伺服器/主機同時處理的session數量
...
def classify_packets(packet_list, session_track):
    grouped_packets = {}
    ...
    session_info = {} # 紀錄每個session的起點、終點、SYN旗標
    for group_key, packets in grouped_packets.items():
        if group_key == 'background':
            continue

        session_info[group_key] = {
            'First Packet Order': int(packets[0].number),
            'Last Packet Order': int(packets[-1].number),
            'SYN Flag': get_tcp_flags(packet.tcp.flags)['SYN'] # 有該旗標的封包，代表是新的正常session
        }

    # TODO: 這裡應該新增，如果某個port被session用過，優先判給他
    # TODO: btw, 這裡的寫法有點醜，之後再想辦法改進 
    if len([key for key in grouped_packets if key != 'background']) > SESSION_NUM: # 有多餘的session，可能是異常，也可能是reconnect
        reconnect_pairs = find_reconnect_sessions(session_info, expected_session_count=SESSION_NUM)
        
        for old_key, new_key in reconnect_pairs.items():
            logging.info(f"Reconnect: {old_key} -> {new_key}")
            grouped_packets[old_key].extend(grouped_packets[new_key]) # 按順序組合兩個session的封包
            del grouped_packets[new_key]
            grouped_packets[new_key] = grouped_packets.pop(old_key) # 重新命名以保留新的session名稱
        
    # if all(val == 'NONE' for val in session_track.values()): # 首次執行，紀錄每個session的起點、終點、SYN旗標
    #     sorted_group_keys = sorted(grouped_packets.keys(), key=lambda k: len(grouped_packets[k]), reverse=True)[:SESSION_NUM]
    ...
    #         grouped_packets[f"{session_name}: {top_session_info}"] = grouped_packets.pop(top_session_info)
    #         logging.info(f"{session_name} assigned to {top_session_info}")
            
    if all(val == 'NONE' for val in session_track.values()): # 首次執行，紀錄每個session的起點、終點、SYN旗標
        sorted_group_keys = sorted(
            [k for k in grouped_packets.keys() if k != 'background'], 
            key=lambda k: len(grouped_packets[k]), 
            reverse=True)[:SESSION_NUM]
        ...
        for session_name, old_key in session_track.items():
            if old_key in grouped_packets: # 這個session沒有變動
                grouped_packets[f"{session_name}: {old_key}"] = grouped_packets.pop(old_key)
                continue
            
            new_key = reconnect_pairs.get(old_key, None) # 這個session有變動，且是reconnect
            if new_key: 
                session_track[session_name] = new_key
                grouped_packets[f"{session_name}: {new_key}"] = grouped_packets.pop(new_key)
            
            else:
                if session_track[session_name] != 'NONE':
                    logging.info(f"Unable to track {session_name}: {old_key}")
                session_track[session_name] = 'NONE'
                
        if any(val == 'NONE' for val in session_track.values()):
            sorted_group_keys = sorted(
                [k for k in grouped_packets.keys() if "Session" not in k and k != 'background'],
                key=lambda k: len(grouped_packets[k]), 
                reverse=True
                )[:SESSION_NUM]
            
            if len(sorted_group_keys): # 有新的session可供配對，若沒有可能處於斷線狀態
                keys = iter(sorted_group_keys)
                
                for session_name, val in session_track.items():
                    if val == 'NONE':
                        try:
                            top_session_info = next(keys)
                            session_track[session_name] = top_session_info
                            grouped_packets[f"{session_name}: {top_session_info}"] = grouped_packets.pop(top_session_info)
...
def main():
    ...
    session_track ={f'Session {i+1}': 'NONE' for i in range(SESSION_NUM)}
    ...
    for packet_list in packet_lists:
        ...
        # 分類封包
        classified_packets = classify_packets(packet_list, session_track)
